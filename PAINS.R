...
server <- function(input, output) {
  observeEvent(input$submit_btn, {
    smi <- isolate(input$smiles_input)
    molrefs <- forEachMol("SMILES", smi, identity)
    # Your SMARTS expressions (p1 to p481)
    smarts_expressions <- c(
      "[#6]-1(-[#6](~[!#6&!#1]~[#6]-[!#6&!#1]-[#6]-1=[!#6&!#1])~[!#6&!#1])=[#6;!R]",
    ...
    results_list <- list()
    for (i in 1:481) {
      result <- smartsSearch_OB(molrefs, smarts_expressions[i])
      results_list[[paste0("P", i)]] <- result
    }
    
    # Define the file name based on the input SMILES
    file_name <- paste0(gsub("[^[:alnum:]]", "_", smi), "_PAINS.csv")
    
    # Write results to CSV with the updated file name
    write.csv(do.call(rbind, results_list), file_name)
    
    output$result_text <- renderText({
      paste("CSV file created:", file_name, ". Check your working directory.")
    })
  })
}
